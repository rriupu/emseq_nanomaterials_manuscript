rule fastqc:
    ...
    threads: 4
    log:
        os.path.join(config["logs_dir"], "{sample}", "{sample}_fastqc_before_trimming.log")
    benchmark:
        os.path.join(config["benchmarks_dir"], "{sample}", "{sample}_fastqc_before_trimming.txt")
    container:
...
rule multiqc:
    ...
    """
    input:
        expand(rules.fastqc.output.html1, sample = SAMPLES)
    output:
